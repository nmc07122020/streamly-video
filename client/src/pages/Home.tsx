...
import {
  Bell, Bookmark, ChevronDown, Film, History, Home as HomeIcon,
  Library, Menu, MoreVertical, Play, Plus, Search, Settings, Video,
  X, Youtube,
} from "lucide-react";
...
const topicLabels: Record<string, string> = { All: "Tất cả", Music: "Âm nhạc", Mixes: "Bản phối", Live: "Trực tiếp", Design: "Thiết kế", Gaming: "Trò chơi", News: "Tin tức", Travel: "Du lịch", "Recently uploaded": "Mới tải lên" };
const primaryNav = [{ label: "Trang chủ", icon: HomeIcon }, { label: "Video ngắn", icon: Film }, { label: "Kênh đăng ký", icon: Youtube }];
const libraryNav = [{ label: "Thư viện", icon: Library }, { label: "Lịch sử", icon: History }, { label: "Video của bạn", icon: Video }, { label: "Xem sau", icon: Bookmark }];
...
export default function Home() {
  ...
  const filterFeed = (items: FeedVideo[]) => items.filter((video) => {
    const categoryLabel = topicLabels[video.category] || video.category;
    const matchesTopic = topic === "All" || topic === "Recently uploaded" || video.category.toLowerCase() === topic.toLowerCase();
