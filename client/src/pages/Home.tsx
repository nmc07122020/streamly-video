...
export default function Home() {
  ...
  const filterFeed = (items: FeedVideo[]) => items.filter((video) => (topic === "All" || topic === "Recently uploaded" || video.category.toLowerCase().includes(topic.toLowerCase())) && (!search.trim() || `${video.title} ${video.creator}`.toLowerCase().includes(search.trim().toLowerCase())));
  const filteredRecommended = filterFeed(demoVideos);
